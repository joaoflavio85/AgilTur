  ...
  return (
    <div>
      ...
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'16px 20px',marginBottom:20,display:'flex',gap:32}}>
        <div><span style={{fontSize:12,color:'var(--text-muted)'}}>TOTAL PENDENTE</span><div style={{fontSize:22,fontWeight:700,fontFamily:'Sora,sans-serif',color:'var(--success)'}}>{fmtCurr(total)}</div></div>
      </div>
      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            ...
            : contas.length === 0 ? <tr><td colSpan={8}><div className="empty-state">Nenhuma conta encontrada.</div></td></tr>
            : contas.map((c) => (
              <tr key={c.id}>
                <td>
